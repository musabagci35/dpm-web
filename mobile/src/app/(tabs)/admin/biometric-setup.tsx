...
import { adminEnrollBiometric } from "@/lib/auth";
 ...
 * or phone/OTP admin login — never offered as a way to sign in for the
 * first time.
 */
export default function AdminBiometricSetupScreen() {
  const { next } = useLocalSearchParams<{ next?: string }>();
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<string | null>(null);

  function goNext() {
    router.replace((next as any) || "/admin");
  }
  ...
  async function handleEnable() {
    ...
    try {
      ...
      await adminEnrollBiometric();
      await markAskedAboutBiometric("admin");
      goNext();
    } catch (err) {
