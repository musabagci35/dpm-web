...
import { SellerAuthError, sellerRequestOtpLogin, sellerVerifyOtpLogin } from "@/lib/marketplaceAuth";
import { shouldOfferBiometricSetup } from "@/lib/biometricAuth";

type Step = "phone" | "code";

export default function SellerPhoneLoginScreen() {
  ...
  async function handleSendCode() {
    ...
    setSubmitting(true);
    try {
      const result = await sellerRequestOtpLogin(phone.trim());
      setMessage(result);
      setStep("code");
    } catch (err) {
      setError(
  ...
  async function handleVerify() {
    ...
    setSubmitting(true);
    try {
      await sellerVerifyOtpLogin(phone.trim(), code.trim());
      setCode("");
    ...
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <View style={styles.card}>
        ...
        <Text style={styles.subtitle}>
          {step === "phone"
            ? "Enter the phone number on your account and we'll text you a one-time code."
