...
/** Same signed-upload pattern as the photo/document uploaders, posted to Cloudinary's /video/upload endpoint. */
export function uploadVideoToCloudinary(
  ...
  return new Promise((resolve, reject) => {
    const name = video.fileName || `vehicle-video-${Date.now()}.mov`;
